...
if exist "logs" (
    ...
    echo  [!] 日志目录不存在
    echo  [INFO] 服务端启动后会自动创建日志目录
)

echo.
set /p logfile=" 输入日志文件名查看详情 (留空返回): "
...
echo.
echo  [2] 修改端口
echo      - 修改 server_config.ini 中的端口设置
echo      - 修改后需要重启服务端
echo.
echo  [3] 查看配置
echo      - 显示当前配置文件内容
...
echo.
echo  [6] 配置文件说明
echo      - server_config.ini - 服务端配置文件
echo      - config.ini        - 客户端配置文件
echo.
echo  ──────────────────────────────────────────────────────────────────────────────
echo  提示: 将TXT文件拖放到客户端EXE上即可开始查重
echo.
pause
goto menu
